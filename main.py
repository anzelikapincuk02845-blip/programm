...
# 🎨 Загрузка премиальной CSS-стилизации из внешнего файла style.qss
...
class WMSApp:
    def __init__(self):
        ...
        # Загрузка отдельных страниц для операций (спроектированных для Qt Designer)
        ...
        # 3. Перестроение кнопок главного меню для добавления "Задачи ТСД"
        self.setup_menu_buttons()

        # 4. Создание QStackedWidget в нижней части главного окна
        self.setup_stacked_widget()

        # 5. Инициализация всех подразделов (страниц)
        ...
        # 7. Запуск стартового окна авторизации (Вход)
        self.login_win.show()
    ...
    def setup_menu_buttons(self):
        """Скрывает исходные кнопки и заменяет их на новый горизонтальный ряд с ТСД кнопкой"""
        ...
        # Создаем новый горизонтальный фрейм
        ...
        # Новые красивые кнопки управления
        ...
        for btn in [self.btn_menu_dashboard, self.btn_menu_rec, self.btn_menu_inv, self.btn_menu_qual, self.btn_menu_ship, self.btn_menu_tsd, self.btn_menu_rep]:
            btn.setCheckable(True)

        self.btn_menu_exit.setStyleSheet("background-color: #ef4444; color: #ffffff;")
    ...
    def setup_stacked_widget(self):
        """Интегрирует QStackedWidget в свободную нижнюю зону"""
        ...
        self.stacked_widget.setStyleSheet("background-color: #13131a; border: 1px solid #2d2d3d; border-radius: 8px;")

        # Добавляем загруженные .ui страницы
    ...
    def log_event(self, message):
        """Запись в системный лог базы данных"""
    ...
    def refresh_dashboard_logs(self):
        ...
        conn = db_manager.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT timestamp, content FROM documents WHERE doc_type = ? ORDER BY id DESC LIMIT 15", ('LOG',))
    ...
    def confirm_receiving(self):
        ...
        # Логируем завершение документа
        cursor.execute("""
            INSERT INTO documents (doc_type, timestamp, content)
            VALUES (?, ?, ?)
        """, ('RECEIVING', datetime.now().strftime("%Y-%m-%d %H:%M:%S"), f"Акт приемки {self.rec_doc_number}. Продукция: {name} (SKU: {sku}), Кол-во: {qty} шт., Размещено в ячейке: {loc}"))
        
        conn.commit()
        conn.close()

        # 🌟 Логи пишем ПОСЛЕ закрытия основного соединения, чтобы избежать взаимной блокировки базы данных (Deadlock)
        if is_new_product:
            self.log_event(f"Приемка: Создан новый товар '{name}' ({sku}), партия {lot}, кол-во {qty} шт. Статус: На проверку качества.")
        else:
    ...
    def save_inventory_item(self):
        ...
        self.inventory_session_data[sku] = {
            "name": name,
        ...
        if has_discrepancies:
            self.lbl_inv_result_msg.setText("⚠️ <b>Есть расхождения с учетными данными!</b> Требуется составить Акт расхождений.")
    ...
    def create_discrepancy_act(self):
        ...
        QMessageBox.information(self.main_win, "Акт сформирован", f"Акт о расхождениях {self.discrepancy_doc_number} успешно сформирован в системе!")
    ...
    def finish_inventory(self):
        ...
        # Логируем завершение в базу документов
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute("""
            INSERT INTO documents (doc_type, timestamp, content)
            VALUES (?, ?, ?)
        ...
        QMessageBox.information(self.main_win, "Инвентаризация завершена", f"Остатки успешно скорректированы в системе!\nСкорректировано позиций: {corrections_made}.")
    ...
    def advance_quality_progress(self):
        ...
        if self.quality_progress >= 100:
            ...
            QMessageBox.information(self.main_win, "Тест завершен", "Лабораторный анализ партии успешно проведен! Пожалуйста, вынесите решение.")
    ...
    def confirm_quality_accept(self):
        ...
        QMessageBox.information(self.main_win, "Утверждено", f"Партия {lot} успешно РАЗБЛОКИРОВАНА и готова к отгрузке!")
    ...
    def confirm_quality_reject(self):
        ...
        # Обновляем статус на BLOCKED и пишем брак в систему
    ...
    def verify_shipping(self):
        ...
        # Валидация запасов и статуса качества в БД
        conn = db_manager.get_connection()
        cursor = conn.cursor()
    ...
    def create_shipping_pick_task(self):
        ...
        QMessageBox.information(self.main_win, "Задача создана", f"Складская задача на отбор для SKU {sku} ({qty} шт.) успешно передана на мобильные ТСД!")
    ...
    def ship_product_instant(self):
        ...
        # Записываем товарный документ
        ship_doc = f"SHIP-ACT-{datetime.now().strftime('%M%S')}"
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute("""
            INSERT INTO documents (doc_type, timestamp, content)
            VALUES (?, ?, ?)
        """, ('SHIPPING', now, f"Оформлена расходная накладная {ship_doc}. Списано: {name} (SKU: {sku}), Кол-во: {qty} шт."))
        
        conn.commit()
        conn.close()

        QMessageBox.information(self.main_win, "Отгрузка проведена", f"Расходный ордер {ship_doc} проведен. Товар физически отгружен со склада!")
        self.log_event(f"Отгрузка: Прямое списание со склада. Списан товар '{name}' ({qty} шт.) по накладной {ship_doc}.")
    ...
    def init_tasks_page(self):
        ...
        self.tbl_tasks.setHorizontalHeaderLabels(["ID Задачи", "Описание складской задачи", "Товар SKU", "Количество", "Текущий статус"])
    ...
    def on_task_selected(self):
        ...
        self.lbl_tsd_task_desc.setText(f"<b>Задача #{self.selected_task_id}:</b> {desc} (SKU: {sku}, Кол-во: {qty} шт.) [Статус: {status}]")
    ...
    def scan_pick_barcode(self):
        ...
        QMessageBox.information(self.main_win, "Успешное сканирование", f"Штрихкод товара SKU {sku} отсканирован ТСД! Найдена ячейка. Товар укомплектован.")
    ...
    def execute_task_completed(self):
        ...
        # Логируем накладную/документ отгрузки
        ship_doc = f"TSD-SHIP-{datetime.now().strftime('%M%S')}"
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute("""
            INSERT INTO documents (doc_type, timestamp, content)
            VALUES (?, ?, ?)
        """, ('SHIPPING', now, f"Отгрузка через ТСД. Оформлена накладная {ship_doc}. Товар SKU: {sku}, списано: {qty} шт."))

        conn.commit()
        conn.close()

        QMessageBox.information(self.main_win, "Успех", f"Задание #{self.selected_task_id} выполнено! Товар упакован, отгружен, остатки скорректированы.")
        self.log_event(f"ТСД: Задача #{self.selected_task_id} успешно ЗАКРЫТА сборщиком. Оформлена накладная {ship_doc}.")
    ...
    def on_report_type_changed(self):
        ...
        if report_type == "STOCK":
            ...
            self.tbl_report.setHorizontalHeaderLabels(["Артикул (SKU)", "Название номенклатуры", "Количество", "Партия", "Статус качества"])
            ...
            self.lbl_report_summary.setText(f"Общее количество товаров на складе: <b>{total_qty} шт.</b> в {len(rows)} партиях.")
            ...
            cursor.execute("SELECT timestamp, doc_type, content FROM documents WHERE doc_type != ? ORDER BY id DESC", ('LOG',))
            rows = cursor.fetchall()
            ...
            self.lbl_report_summary.setText(f"Общий объем бракованной/блокированной продукции: <font color='#ef4444'><b>{total_blocked} шт.</b></font>")
        ...
        try:
            with open(filename, "w", encoding="utf-8") as f:
                ...
                if report_type == "STOCK":
                    ...
                    rows = cursor.fetchall()
                    f.write(f"{'SKU':<8} | {'Наименование':<32} | {'Кол-во':<6} | {'Партия':<14} | {'Статус':<10} | {'Ячейка':<6}\n")
                    f.write("-" * 85 + "\n")
                    ...
                    cursor.execute("SELECT sku, name, qty, lot_number, location FROM products WHERE quality_status = ?", ('BLOCKED',))
                    rows = cursor.fetchall()
